...
public class EmitterPositionController : MonoBehaviour
{
    ...
    {
        // Check if the object that triggered the collider has the correct tag
        if (other.CompareTag(targetTag))
        {
            // If the effect is a particle system, stop and disable it
            if (effect != null)
            {
                effect.Stop();
            ...
            {
                // If it's a different kind of effect, just disable the GameObject
